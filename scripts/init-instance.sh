...
read -p "Public Party ID (leave blank to skip): " PUBLIC_PARTY

echo ""
echo -e "${YELLOW}Optional Features:${NC}"
read -p "Chat Agent Webhook URL (leave blank to skip): " CHAT_AGENT_WEBHOOK_URL

read -p "Enable MPC Signer? (y/n) [n]: " MPC_ENABLED
MPC_ENABLED=${MPC_ENABLED:-n}
MPC_AUTH_SECRET=""
MPC_SERVICE_NAME=""
if [ "$MPC_ENABLED" = "y" ] || [ "$MPC_ENABLED" = "Y" ]; then
  read -sp "MPC Auth Secret: " MPC_AUTH_SECRET
  echo ""
  read -p "MPC Signer service name [stratos-mpc-signer]: " MPC_SERVICE_NAME
  MPC_SERVICE_NAME=${MPC_SERVICE_NAME:-stratos-mpc-signer}
fi
...
if [ -n "$PUBLIC_PARTY" ]; then
  echo "Public Party:      ${PUBLIC_PARTY}"
fi
if [ -n "$CHAT_AGENT_WEBHOOK_URL" ]; then
  echo "Chat Agent:        ${CHAT_AGENT_WEBHOOK_URL}"
fi
if [ "$MPC_ENABLED" = "y" ] || [ "$MPC_ENABLED" = "Y" ]; then
  echo "MPC Signer:        enabled (${MPC_SERVICE_NAME})"
fi
...
STEP=$((STEP + 1))
echo ""
echo -e "${GREEN}[${STEP}/${TOTAL_STEPS}] Copying project files to ${TARGET_DIR}...${NC}"
...
STEP=$((STEP + 1))
echo ""
echo -e "${GREEN}[${STEP}/${TOTAL_STEPS}] Creating D1 database '${DB_NAME}'...${NC}"
...
STEP=$((STEP + 1))
echo ""
echo -e "${GREEN}[${STEP}/${TOTAL_STEPS}] Generating wrangler.toml...${NC}"

cat > "$TARGET_DIR/wrangler.toml" << WRANGLER_EOF
...
# Append optional vars
if [ -n "$PUBLIC_PARTY" ]; then
  echo "PUBLIC_PARTY = \"${PUBLIC_PARTY}\"" >> "$TARGET_DIR/wrangler.toml"
fi

if [ -n "$CHAT_AGENT_WEBHOOK_URL" ]; then
  echo "CHAT_AGENT_WEBHOOK_URL = \"${CHAT_AGENT_WEBHOOK_URL}\"" >> "$TARGET_DIR/wrangler.toml"
fi

if [ "$MPC_ENABLED" = "y" ] || [ "$MPC_ENABLED" = "Y" ]; then
  echo "MPC_AUTH_SECRET = \"${MPC_AUTH_SECRET}\"" >> "$TARGET_DIR/wrangler.toml"
  cat >> "$TARGET_DIR/wrangler.toml" << MPC_EOF

# MPC Signer Service Binding
[[services]]
binding = "MPC_SIGNER"
service = "${MPC_SERVICE_NAME}"
MPC_EOF
fi
...
STEP=$((STEP + 1))
echo ""
echo -e "${GREEN}[${STEP}/${TOTAL_STEPS}] Applying database schema...${NC}"
...
STEP=$((STEP + 1))
echo ""
echo -e "${GREEN}[${STEP}/${TOTAL_STEPS}] Creating superadmin user...${NC}"
...
STEP=$((STEP + 1))
echo ""
echo -e "${GREEN}[${STEP}/${TOTAL_STEPS}] Seeding default assets and RPC endpoints...${NC}"
...
STEP=$((STEP + 1))
echo ""
echo -e "${GREEN}[${STEP}/${TOTAL_STEPS}] Installing npm dependencies...${NC}"
...
# Step 8 (conditional): Set up MPC signer worker
if [ "$MPC_ENABLED" = "y" ] || [ "$MPC_ENABLED" = "Y" ]; then
  STEP=$((STEP + 1))
  echo ""
  echo -e "${GREEN}[${STEP}/${TOTAL_STEPS}] Setting up MPC signer worker...${NC}"

  MPC_DIR="${TARGET_DIR}/workers/mpc-signer"
  if [ ! -d "$MPC_DIR" ]; then
    echo -e "${RED}Error: workers/mpc-signer not found in target directory.${NC}"
  else
    # Update MPC signer wrangler.toml with instance-specific values
    cat > "$MPC_DIR/wrangler.toml" << MPC_WRANGLER_EOF
name = "${MPC_SERVICE_NAME}"
...
[durable_objects]
bindings = [{ name = "MPC_KEY_STORE", class_name = "MpcKeyStore" }]

[[migrations]]
tag = "v1"
new_classes = ["MpcKeyStore"]
MPC_WRANGLER_EOF

    echo -e "${GREEN}MPC signer wrangler.toml configured.${NC}"

    # Install MPC signer dependencies
    echo "Installing MPC signer dependencies..."
    cd "$MPC_DIR"
    npm install
    cd "$TARGET_DIR"

    echo -e "${GREEN}MPC signer worker ready.${NC}"
  fi
fi
...
if [ "$DEPLOY_NOW" = "y" ] || [ "$DEPLOY_NOW" = "Y" ]; then
  # Deploy MPC signer first (main wallet depends on it via service binding)
  if [ "$MPC_ENABLED" = "y" ] || [ "$MPC_ENABLED" = "Y" ]; then
    MPC_DIR="${TARGET_DIR}/workers/mpc-signer"
    if [ -d "$MPC_DIR" ]; then
      echo ""
      echo -e "${GREEN}Deploying MPC signer worker...${NC}"
      cd "$MPC_DIR"
      wrangler deploy || {
        echo -e "${YELLOW}MPC signer deployment may have failed. You can retry manually:${NC}"
        echo "  cd ${MPC_DIR} && wrangler deploy"
      }
      cd "$TARGET_DIR"
    fi
  fi
